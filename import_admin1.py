...
# Path to the CSV file for Admin1
CSV_FILE_PATH = "C:\\Users\\HP\\AndroidStudioProjects\\ChatApp\\Database.TWAPP - Admin_ 1 (Second_Year).csv"

def import_admin1_to_firestore():
    """Import Admin1 data from CSV to Firestore"""
    print("Starting import of Admin1 data to Firestore...")
    ...
    try:
        with open(CSV_FILE_PATH, 'r', encoding='utf-8') as csv_file:
            ...
            print(f"Found {len(rows)} Admin1 records in CSV file")
            ...
            for batch_index in range(total_batches):
                ...
                print(f"Processing batch {batch_index + 1}/{total_batches} ({len(batch_rows)} Admin1 records)")
                ...
                for row in batch_rows:
                    try:
                        ...
                        # Add user type field (Admin1)
                        admin_data["userType"] = "Admin1"
                        
                        # Create a reference to the Admin1 document
                        admin_ref = db.collection('Admin1').document(roll_no)
                        
                        # Add to batch
                        batch.set(admin_ref, admin_data)
                    except Exception as e:
                        print(f"Error processing Admin1 record {roll_no}: {e}")
            ...
            print(f"Import completed. Successfully imported {success_count} out of {len(rows)} Admin1 records.")
