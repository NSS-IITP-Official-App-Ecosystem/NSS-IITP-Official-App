...
exports.onChatMessage = onDocumentCreated(
  ...
  async (event) => {
    try {
      ...
      const senderId = message.sender || 'Someone';
      ...
      try {
        ...
        console.warn('Could not fetch group details:', err);
      }
      
      // Remove the sender so they don't get a notification for their own message
      participants = participants.filter(id => id !== senderId);
      ...
      // The Android app subscribes each user to 'user_$userId'
      // Send a push notification to each participant's personal topic
      const messaging = admin.messaging();
      const sendPromises = participants.map(participantId => {
        const payload = {
          notification: {
          ...
          topic: `user_${participantId}`
        };
        return messaging.send(payload).catch(err => {
          console.warn(`Failed to notify user_${participantId}:`, err);
        });
      });
      
      await Promise.all(sendPromises);
      console.log(`Chat notifications sent to ${participants.length} users for message in ${groupName}`);
